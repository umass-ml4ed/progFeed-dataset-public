...
def delete_task(list, task):
    list.remove(task)
    return 'Task successfully deleted. '+str(len(list))+' tasks remaining.'

def move_task(list, old_index, new_index):
    list.insert(new_index, list.pop(old_index))
    return 'Task successfully moved. '+str(len(list))+' tasks remaining.'
# def move_task(list,from_index,to_index):
#   list.insert(to_index,list.pop(from_index))
#   return print(f"Task '{list[to_index]}' successfully moved to index {to_index}")
# ===== YOUR CODE ENDS HERE =====

...
class TestExpressionHW(unittest.TestCase):
  ...
  def test_average_scores(self):
      """average_scores() test"""
      mod, fun = "average_scores", "average_scores"
      print(f"*** Testing {fun}() ***\n")
      self.require_function(their, "average_scores")

      passed = 0

      # 🔒 10 fixed, strong test cases (covering various lateness penalties, lengths, and value ranges)
      test_cases = [
          # 1️⃣ simple single student all on-time
          ([[ (100, 0), (80, 0), (90, 0) ]], [90.0]),

          # 2️⃣ single student all late once (0.9)
          ([[ (100, 1), (80, 1), (60, 1) ]], [72.0]),

          # 3️⃣ single student mix of lateness
          ([[ (100, 0), (80, 1), (60, 2), (40, 3) ]], [(100*1 + 80*0.9 + 60*0.75 + 40*0.5)/4]),
          
          # 4️⃣ multiple students, varying lateness
          ([
              [(100, 0), (50, 1), (0, 2)], 
              [(90, 0), (70, 0), (50, 1), (30, 3)]
          ],
          [
              (100 + 50*0.9 + 0*0.75)/3,              
              (90 + 70 + 50*0.9 + 30*0.5)/4
          ]),

          # 5️⃣ student with lateness=4 (no credit)
          ([[ (100, 4), (50, 4), (75, 0) ]],
          [(100*0 + 50*0 + 75*1)/3]),

          # 6️⃣ mix of zeros and lateness penalties
          ([
              [(0, 0), (0, 1), (0, 3)], 
              [(100, 1), (50, 2)]
          ],
          ...
          # 7️⃣ two students, high variance
          ([
              [(100, 0), (100, 0), (100, 0)],
              [(10, 3), (20, 3), (30, 3)]
          ],
          [
              (100 + 100 + 100)/3,
              (10*0.5 + 20*0.5 + 30*0.5)/3
          ]),

          # 8️⃣ 4 scores, mixed lateness and zeros
          ([[ (0, 0), (40, 2), (80, 1), (100, 3) ]],
          [(0 + 40*0.75 + 80*0.9 + 100*0.5)/4]),

          # 9️⃣ all lateness=2
          ([[ (100, 2), (80, 2), (60, 2) ]],
          [(100*0.75 + 80*0.75 + 60*0.75)/3]),
          # 🔟 larger student list with mixed cases
          ([
              [(90, 0), (80, 1), (70, 2), (60, 3), (50, 4)],
              [(100, 0), (100, 0), (100, 0)]
          ],
          [
              (90 + 80*0.9 + 70*0.75 + 60*0.5 + 50*0)/5,
              100.0
          ]),
      ]

      for i, (all_scores, expected) in enumerate(test_cases, start=1):
          result = their.average_scores(all_scores)
          result_rounded = [round(x, 2) for x in result]
          expected_rounded = [round(x, 2) for x in expected]
          ...
          if result_rounded == expected_rounded:
              ...
              print(f"❌ Test case {i} failed")
              debug_print(f"    Input: {all_scores}")
              debug_print(f"    Expected: {expected_rounded}, but got: {result_rounded}")
      ...
      if passed != len(test_cases):
          ...
          if student_email and student_consented(student_email):
              hinttype = student_experiment_type(student_email)
              feedback = generate_ai_feedback(
                  problem_description_file="average_scores_desc.txt",
                  file_name="nested_loops.py",
      ...
      print("✅ average_scores seems good")
      print()

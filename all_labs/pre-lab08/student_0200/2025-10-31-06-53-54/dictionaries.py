...
def greet_user(users, user_id, default_lang="en", default_age=18):
    ...
    langauge = user_pref.get('langauge', default_lang)
    age = user_pref.get('age', default_age)
    greeting = {"en": (f"Hello, {name}!", f"Hey there, {name}!"), "es": (f"Hola, {name}!", "¡Hola, pequeño/a {name}!"), "fr": ("Bonjour, {name}!", "Salut, {name}!")}
    if langauge not in greeting:
        langauge = default_lang
    adult_greet, child_greet = greeting[langauge]
    if age < 18:
        return child_greet
    else:
        return adult_greet

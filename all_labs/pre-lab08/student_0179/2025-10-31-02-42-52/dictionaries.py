...
def greet_user(users,user_id,default_lang = 'en', default_age = 18):
    ...
    if users[user_id]["language"] == "en" and (users[user_id]["age"]) >= 18:
        return (f"Hello, {users[user_id]["name"]}!")
    elif users[user_id]["language"] == "en" and (users[user_id]["age"]) < 18:
        return(f"Hey there, {users[user_id]["name"]}!")
    elif users[user_id]["language"] == "es" and (users[user_id]["age"]) >= 18:
        return (f"Hola, {users[user_id]["name"]}!")
    elif users[user_id]["language"] == "es" and (users[user_id]["age"]) < 18:
        return(f"¡Hola, pequeño/a {users[user_id]["name"]}!")
    elif users[user_id]["language"] == "fr" and (users[user_id]["age"]) >= 18:

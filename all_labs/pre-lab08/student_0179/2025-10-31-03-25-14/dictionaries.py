...
def greet_user(users,user_id,default_lang = "en", default_age = 18):
    ...
    if "language" not in new_users[user_id]:
        new_users[user_id]["language"] = default_lang
    if "age" not in users[user_id]:
        new_users[user_id]["age"] = default_age
    if new_users[user_id]["language"] == "en" and new_users[user_id]["age"] >= 18:
        return (f"Hello, {users[user_id]["name"]}!")
    elif new_users[user_id]["language"] == "en" and new_users[user_id]["age"] < 18:
        return(f"Hey there, {users[user_id]["name"]}!")
    elif new_users[user_id]["language"] == "es" and new_users[user_id]["age"] >= 18:
        return (f"Hola, {users[user_id]['name']}!")
    elif new_users[user_id]["language"] == "es" and new_users[user_id]["age"] < 18:
        return(f"¡Hola, pequeño/a {users[user_id]["name"]}!")
    elif new_users[user_id]["language"] == "fr" and new_users[user_id]["age"] >= 18:

...
to_do_list = ['Study for physics', 'Go to Totman gym', 'Make dinner']
...
def delete_task(to_do_list, task):
    to_do_list.remove(task)              # remove the given task
    n = len(to_do_list)                  # count after removal
    return f"Task successfully deleted. {n} tasks remaining."
def move_task(to_do_list, from_index, to_index):
    task = to_do_list.pop(from_index)     # take the item out
    to_do_list.insert(to_index, task)     # put it at the new index
    return f"Task '{task}' successfully moved to index {to_index}"

to_do_list = []
print(add_task(to_do_list, 'Study for physics'))
print(add_task(to_do_list, 'Go to Totman gym'))
print(add_task(to_do_list, 'Make dinner'))
print(move_task(to_do_list, 2, 0))
print(to_do_list) 
print(move_task(to_do_list, 1, 2))
print(to_do_list) 
print(delete_task(to_do_list, 'Go to Totman gym'))     
print(delete_task(to_do_list, 'Make dinner'))  

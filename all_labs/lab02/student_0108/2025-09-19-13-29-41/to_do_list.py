...
def add_task(lst_add, descrription_add):
  lst_add.append(descrription_add)
  return ("Task successfully added. " + str(len(lst_add)) + " tasks remaining.")

def delete_task(lst_del, description_del):
  lst_del.remove(description_del)
  return ("Task successfully deleted. " + str(len(lst_del)) + " tasks remaining.")

def move_task(lst_move, from_index, to_index):
  lst_move.insert(to_index, lst_move.pop(from_index))
  return("Task \'"+ lst_move[to_index]+ "\' successfully moved to index " + str(to_index))

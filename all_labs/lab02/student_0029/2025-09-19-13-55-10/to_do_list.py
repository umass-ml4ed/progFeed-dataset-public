...
def add_task(to_do_list, task):
  ...
  return f"Task successfully added. {str(len(to_do_list))} tasks remaining."

def delete_task(to_do_list, task):
  if task in to_do_list:
    to_do_list.remove(task)
    return f"Task successfully deleted. {str(len(to_do_list))} tasks remaining."
  else:
    return f"Task not found."

def move_task(to_do_list, from_index, to_index):
  if from_index < 0 or from_index >= len(to_do_list) or to_index < 0 or to_index >= len(to_do_list):
    return f"invalid index"
  task = to_do_list.pop(from_index)
  to_do_list.insert(to_index, task)
  return f"Task '{str(task)}' successfully moved to index {str(to_index)}"
...
#ncomment the following 5 lines (i.e. remove the # characters on each line) to test add_task

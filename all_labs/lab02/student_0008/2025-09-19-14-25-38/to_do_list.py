...
def add_task(list, task):
    list.append(task)
    print("Task succesfully added.", len(list), "tasks remaining.")

def delete_task(list, task):
    list.remove(task)
    print("Task succesfully deleted.", len(list), "tasks remaining.")

def move_task(list, from_index, to_index):
    task = list.pop(from_index)
    list.insert(to_index, task)
    print(f"Task '{task}' successfully moved to index {to_index}")
# ===== YOUR CODE ENDS HERE =====

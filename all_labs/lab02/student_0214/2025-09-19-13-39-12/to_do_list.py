...
def add_task(list, task):
  list.append(task)
  remaining = len(list)
  return (f"Task succesfully added. {remaining} tasks remaining.")

def delete_task(list, task):
  list.remove(task)
  remaining = len(list)
  return (f"Task succesfully deleted. {remaining} tasks remaining.")

def move_task(list, from_index, to_index):
  task = list.pop(from_index)
  list.insert(to_index, task)
  return f"Task '{task}' succefully moved to index {to_index}"

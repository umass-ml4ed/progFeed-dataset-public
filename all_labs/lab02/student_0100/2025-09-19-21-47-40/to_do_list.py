...
def add_task(a_list, to_add):
  a_list.append(to_add)
  tasks_remaining = len(a_list)
  result = f"Task successfully added. {tasks_remaining} tasks remaining."
  return result

def delete_task(a_list, to_delete):
  a_list.remove(to_delete)
  tasks_remaining = len(a_list)
  result = f"Task successfully deleted. {tasks_remaining} tasks remaining."
  return result

def move_task(a_list, from_index, to_index):
  initial_index = a_list[to_index]
  a_list[to_index] = a_list[from_index]
  a_list[from_index] = initial_index
  result = f"Task \'{a_list[to_index]}\' successfully moved to index {to_index}"
  return result

...
def add_task(to_do_list, to_add):
  to_do_list.append(to_add)
  tasks_remaining = len(to_do_list)
  return f"Task successfully added. {tasks_remaining} tasks remaining."

def delete_task(to_do_list, to_delete):
...
def move_task(to_do_list, from_index, to_index):
  initial_index = to_do_list[to_index]
  to_do_list[to_index] = to_do_list[from_index]
  to_do_list[from_index] = initial_index
  return f"Task '{to_do_list[to_index]}' successfully moved to index {to_index}"

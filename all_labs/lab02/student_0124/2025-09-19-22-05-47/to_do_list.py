...
def add_task(list, addtask):
  list.append(addtask)
  return 'Task successfully added. ' +  str(len(list)) + ' tasks remaining.'

def delete_task(list, deltask):
  list.remove(deltask)
  return 'Task successfully deleted. '+ str(len(list)) + ' tasks remaining.'

def move_task (list, inow, iwant):
  item = list.pop(inow)
  list.insert(iwant, item)
  return 'Task '+ "'" + str(item) + "' " + 'successfully moved to index ' + str(iwant)
# ===== YOUR CODE ENDS HERE =====

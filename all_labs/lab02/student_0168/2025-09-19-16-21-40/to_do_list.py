...
x=to_do_list
def add_task(x, y):
  to_do_list.append(to_do_list)
  a=len(to_do_list)
  return('Task successfully added. ' + str(a) + ' tasks remaining.')


def delete_task(x, z):
  to_do_list.remove(to_do_list)
  return('Task succsessfully deleted. '+ str(len(to_do_list)) + " tasks remaining.")


def move_task(x, from_index, to_index):
  b=x.pop(from_index)
  x.insert(to_index, b)
  return("Task", b, "successfully move to index ", to_index)

...
def add_task(x, task):
  x.append(task)
  a=len(x)
  return('Task successfully added. ' + str(a) + ' tasks remaining.')


def delete_task(x, task):
  x.remove(task)
  return('Task succsessfully deleted. '+ str(len(x)) + " tasks remaining.")


def move_task(x, from_index, to_index):
  task=x.pop(from_index)
  x.insert(to_index, task)
  return("Task", str(task), "successfully move to index ", to_index)

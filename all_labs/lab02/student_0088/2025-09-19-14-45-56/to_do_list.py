...
def add_task(to_do_list,new):
  to_do_list.append(new)
  tasks_a = len(to_do_list)
  return "Task successfully added. "+str(tasks_a)+" tasks remaining."

def delete_task(to_do_list,delete):
  to_do_list.remove(delete)
  tasks_d = len(to_do_list)
  return "Task successfully deleted. "+str(tasks_d)+" tasks remaining"

def move_task(to_do_list,from_index,to_index):
  x = to_do_list.pop(from_index)
  to_do_list.insert(to_index, x)
  return 'Task '+str(x)+' successfully moved to index '+str(to_index)

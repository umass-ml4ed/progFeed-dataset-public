...
def add_task(list,input):
  list.append(input)
  return ("Task successfully added. " + str(len(list)) + " tasks remaining.")

def delete_task(list2,input2):
  list2.remove(input2)
  return ("Task successfully deleted. " + str(len(list2)) + " tasks remaining.")


def move_task(list3,whereitis,whereyawantit):
  whereyaactuallywantit = whereyawantit -2
  thing = list3.pop(whereitis)
  list3.insert(whereyaactuallywantit,str(thing))
  return ("Task " + str(whereitis) + " sucsessfully moved to index " + str(whereyawantit))

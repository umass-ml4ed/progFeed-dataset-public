...
def add_task(to_do_list, what_task):
  to_do_list.append(what_task)
  add_acknowledgement = "Task successfully added. " + str(len(to_do_list)) + " tasks remaining."
  return add_acknowledgement

def delete_task(to_do_list, delete_dat):
  to_do_list.remove(delete_dat)
  delete_acknowledgement = "Task successfully deleted. " + str(len(to_do_list)) + " tasks remaining."
  return delete_acknowledgement

def move_task(to_do_list, from_index, to_index):
    fptask = to_do_list.pop(from_index) 
    to_do_list.insert(to_index, fptask) 

...
def add_task(to_do_list, task):
    ...
    return "Task added successfully. " + str(len(to_do_list)) + " tasks remaining."

def delete_task(to_do_list, task):
   if task in to_do_list:
        to_do_list.remove(task)
        return "Task deleted successfully. " + str(len(to_do_list)) + " tasks remaining."
   
def move_task(to_do_list, from_index, to_index):
    to_do_list[to_index], to_do_list[from_index] = to_do_list[from_index], to_do_list[to_index]
    return "Task '" + str(to_do_list[from_index]) + "' successfully moved to index " + str(to_index) + "."
# ===== YOUR CODE ENDS HERE =====

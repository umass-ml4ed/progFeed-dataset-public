...
def add_task(list_object,task_object):
  list_object.append(task_object)
  n = len(list_object)
  return str(f"Task successfully added. {n} tasks remaining")

def delete_task(list_object,delete_object):
  list_object.remove(delete_object)
  n = len(list_object)
  return str(f"Task successfully deleted. {n} tasks remaining")

def move_task(list_object,from_index,to_index):
  #to_do_list.pop(from_index)
  task = list_object.pop(from_index)
  list_object.insert(to_index,task)
  return str(f"Task {task} successfully moved to index {to_index}")

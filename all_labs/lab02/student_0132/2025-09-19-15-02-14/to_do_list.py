...
def add_task(to_do_list,task):
    ...
    N=str(len(to_do_list))
    return str("Task successfully added. " + N + " tasks remaining.")

def delete_task(to_do_list, task):
    to_do_list.remove(task)
    N=str(len(to_do_list))
    return str("Task successfully removed. " + N + " tasks remaining.")

def move_task(to_do_list, from_index, to_index):
   to_do_list.insert(to_index,to_do_list.pop(from_index))
   T = str(to_do_list[to_index]); J = str(to_index)
   return str("\'"+"Task " + "\'"+ T + "\'" + " successfully moved to index " + J+ "\'")
...
print(delete_task(['Buy milk', 'Read book', 'Write code'], 'Read book'))
print(move_task(["Task 1", "Task 2", "Task 3"], 1, 2))
#print(add_task([], "Finish homework"))

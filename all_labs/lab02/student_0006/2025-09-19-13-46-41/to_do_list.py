...
def add_task(to_do_list, string):
   to_do_list.append(string)
   N = len(to_do_list)
   return f"Task successfully added. {N} tasks remaining."

def delete_task(to_do_list, string):
   to_do_list.remove(string)
   N = len(to_do_list)
   return f"Task successfully added. {N} tasks remaining."

def move_task(to_do_list, T, J):
   from_index = to_do_list.pop(T)
   to_do_list.insert(J,from_index)
   return f"Task {from_index} successfully moved to index {J}"

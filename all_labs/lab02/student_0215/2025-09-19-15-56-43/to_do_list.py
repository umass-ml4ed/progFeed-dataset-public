...
def add_task(to_do_list, task):
  ...
  output_str = f"Task successfully added. {len(to_do_list)} tasks remaining."
  return output_str
  
def delete_task(to_do_list, task):
  to_do_list.remove(task)
  output_str_2 = f"Task successfully deleted. {len(to_do_list)} tasks remaining."
  return output_str_2

def move_task(to_do_list, from_index, to_index):
  to_do_list.insert(to_index, to_do_list.pop(from_index))
  output_str_3 = f"Task '{to_do_list[to_index]}' successfully moved to index {to_index}"
  return output_str_3

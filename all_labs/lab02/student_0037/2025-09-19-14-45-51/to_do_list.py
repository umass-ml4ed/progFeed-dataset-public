...
def add_task(to_do_list, task):
  ...
  return str(f'Task successfully added. {len(to_do_list)} tasks remaining.')

def delete_task(to_do_list, task):
  to_do_list.remove(task)
  return str(f'Task successfully deleted. {len(to_do_list)} tasks remaining.')

def move_task(to_do_list,from_index,to_index):
  t = to_do_list.pop(from_index)
  to_do_list.insert(to_index,t)
  return str(f'Task {t} has been successfully moved to index {to_index}')

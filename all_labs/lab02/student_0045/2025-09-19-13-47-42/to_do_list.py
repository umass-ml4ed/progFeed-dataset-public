...
def add_task(to_do_list, task_description):
...
def delete_task(to_do_list, task_description):
  to_do_list.remove(task_description)
  print(f"Task successfully deleted. {len(to_do_list)} tasks remaining.")
  return task_description

def move_task(to_do_list, from_index, to_index):
  task_moved = to_do_list.pop(from_index)
  to_do_list.insert(to_index, task_moved)
  print(f"Task {to_do_list[to_index]} has been moved to {to_index}")
  return task_moved
  #print(f"Task {from_index} successfully moved to index {to_index}")

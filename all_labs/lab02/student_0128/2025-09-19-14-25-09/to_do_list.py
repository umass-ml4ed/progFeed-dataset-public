...
def add_task(the_list, the_task):
  the_list.append(the_task)
  tasks_remaining = f"Task successfully added. {str(len(the_list))} tasks remaining."
  return tasks_remaining

def delete_task(the_list, the_task):
  the_list.remove(the_task)
  tasks_remaining = f"Task successfully deleted. {str(len(the_list))} tasks remaining."
  return tasks_remaining

def move_task(the_list, from_index, to_index):
  to_be_moved = the_list[from_index]
  the_list.pop(from_index)
  the_list.insert(to_index, to_be_moved)
  return f"Task '{to_be_moved}' successfully moved to index {to_index}"

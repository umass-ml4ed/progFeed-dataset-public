...
def add_task(to_do_list, task):
  ...
  return "Task successfully added. " + str(length) + " tasks remaining."
print(add_task(to_do_list, "play 67"))

def delete_task(to_do_list, task):
  to_do_list.remove(task)
  length = len(to_do_list)
  return "Task successfully deleted. " + str(length) + " tasks remaining."
print(delete_task(to_do_list, "play 67"))


def move_task(to_do_list, from_index, to_index):
  #to_do_list = [67, 68, 69, 70]
  t = to_do_list.pop(from_index)
  to_do_list.insert(to_index,t)
  return "Task '"+ str(t)+ "' successfully moved to index "+ str(to_index)
#print(move_task(to_do_list, 2, 1))
# ===== YOUR CODE ENDS HERE =====

...
def add_task(List, Task):
  List.append(Task)
  text = "Task successfully added. " + str(len(List)) + " tasks remaining."
  #print(f)
  return text 

def delete_task(List, Task):
  List.remove(Task)
  text = str("Task successfully deleted. " + str(len(List)) + " tasks remaining.")
  #print(text)
  #Expected output: Task successfully removed. 2 tasks remaining.
  return text

def move_task(List, From, To):
  List.insert(To, List.pop(From))
  text = "Task '" + List[To] + "' successfully moved to index " + str(To)
  return text

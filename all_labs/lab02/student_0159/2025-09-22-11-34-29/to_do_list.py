...
def add_task(todo, description):
  todo.append(description)
  return f'Task successfully added. {len(todo)} tasks remaining.'

def delete_task(todo, description):
  if description in todo:
    todo.remove(description)
    return f'Task successfully deleted. {len(todo)} tasks remaining.'
  
def move_task(todo, from_index,to_index):
  words = todo[from_index]
  todo.pop(from_index)
  todo.insert(words, to_index)
  return f'Task {words} successfully moved to indexx {to_index}'

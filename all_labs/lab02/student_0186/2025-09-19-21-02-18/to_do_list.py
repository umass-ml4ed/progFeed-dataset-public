...
def add_task(to_do_list,d):
  to_do_list.append(d)
  x=len(to_do_list)
  return f"Task succesfully added. {x} tasks remaining "
...
def delete_task(to_do_list,c):
  to_do_list.remove(c)
  z=len(to_do_list)
  return f"Task succesfully removed. {z} tasks remaining "
#print(delete_task(to_do_list, 'zybook reading'))

def move_task(to_do_list,a,b):
  t=to_do_list.pop(a)
  to_do_list.insert(b,t)
  u=len(to_do_list)
  h=to_do_list(b)
  return f"Task {t} Succesfully moved to index {b} "

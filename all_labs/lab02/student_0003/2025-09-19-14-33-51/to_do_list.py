...
def add_task(to_do_list:list, task:str)->list:
  to_do_list.append(task)
  N=len(to_do_list)
  return f"Task successfully added. {N} tasks remaining."

def delete_task(to_do_list:list, task:str)->list:
  to_do_list.remove(task)
  N=len(to_do_list)
  return f"Task successfully deleted. {N} tasks remaining."

def move_task(to_do_list:list, from_index:int, to_index:int)->list:
  removing=to_do_list[from_index]
  to_do_list.remove(to_do_list[from_index])
  to_do_list.insert(to_index, removing)
  return f"Task '{removing}' successfully moved to index {to_index}"

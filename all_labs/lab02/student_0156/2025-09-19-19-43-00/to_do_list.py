...
def add_task(list, a):
  list.append(a)
  return("Task sucessfully added. " + str(len(to_do_list)) + " tasks remaining.")

def delete_task(list, b):
  list.remove(b)
  return("Task sucessfully deleted. " +str(len(to_do_list)) + "tasks remaining.")

def move_task(list, c, d):
  from_index = list.pop(c)
  to_index = list.insert(d, from_index)
  return("Task" + from_index + " sucessfully moved to index " + str(d))

...
def add_task(to_do_list, task_name):
   to_do_list.append(task_name)
   length = str(len(to_do_list))
   return ("Task successfully added. " + length + " tasks remaining.")
def delete_task(to_do_list, task_name):
   to_do_list.remove(task_name)
   length = str(len(to_do_list))
   return ("Task successfully deleted. " + length + " tasks remaining.")
def move_task(to_do_list, from_index, to_index):
   print("to do list from index is", to_do_list[from_index])
   to_do_list.insert(to_index, to_do_list.pop(from_index))
   return ("Task " + to_do_list[to_index]+ " successfully moved to index " + str(to_index))

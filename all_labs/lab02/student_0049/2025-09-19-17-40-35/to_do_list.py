...
def add_task(list, item_to_add):
  list.append(item_to_add)
  n = len(list)
  return f'Task successfully added. {n} tasks remaining.'

def delete_task(list, item_to_delete,):
  list.remove(item_to_delete)
  t = len(list)
  return f'Task successfully deleted. {t} tasks remaining.'

def move_task(list, from_index, to_index):
  list[from_index], list[to_index] = list[to_index], list[from_index]
  return f'Task \'{list[from_index]}\' successfully moved to index {to_index}'
# ===== YOUR CODE ENDS HERE =====

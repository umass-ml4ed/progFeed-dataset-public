...
def delete_task(to_do_list, task):
  ...
  return "Task sucesssfully deleted. " + str(len(to_do_list)) + " tasks remaining"

def move_task(to_do_list, from_index, to_index):
  value = to_do_list.pop(from_index)
  temp = value
  to_do_list.insert(to_index, temp)
  return "Task " + "'" + str(temp) + "'" + " successfully moved to index " + str(to_index) 

...
def add_task(list, x):
  list.append(x)
  n = len(list)
  m = f'Task successfully added. {n} tasks remaining'
  return m

def delete_task(list, x):
  list.remove(x)
  n = len(list)
  m = f'Task successfully deleted. {n} tasks remaining'
  return m

def move_task(list, from_index, to_index):
  v = list.pop(from_index)
  list.insert(to_index, v)
  m = f"Task '{v}' successfully moved to index {to_index}"
  return m

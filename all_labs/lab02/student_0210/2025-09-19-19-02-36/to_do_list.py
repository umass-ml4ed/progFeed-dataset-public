...
def add_task(list,text):
  list.append(text)
  total_tasks = len(list)
  return 'Task added successfully. '+ str(total_tasks)+' tasks remaining'

def delete_task(list,text):
  list.remove(text)
  total_tasks = len(list)
  return 'Task removed successfully. '+ str(total_tasks)+' tasks remaining'

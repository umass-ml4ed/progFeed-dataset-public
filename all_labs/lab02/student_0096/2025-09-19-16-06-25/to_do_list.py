...
def add_task(to_do_list,b):
  to_do_list.append(b)
  return 'Task successfully added. ' +str(len(to_do_list))+' task remaining.'

def delete_task(to_do_list,c):
  to_do_list.remove(c)
  return 'Task successfully removed.',str(len(to_do_list)),'task remaining.'

def move_task(to_do_list,d,e):
  aa=to_do_list.pop(d)
  to_do_list.insert(e,aa)
  #to_do_list.insert(d,aa)
  return ('Task '+str(aa)+'successfully moved to index'+str(e))

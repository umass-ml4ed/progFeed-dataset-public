...
def add_task(add_list, task):
  add_list.append(task)
  size = len(add_list)
  return "Task successfully added. " + str(size) + " tasks remaining."
  
def delete_task(delete_list, task_done):
  delete_list.remove(task_done)
  size = len(delete_list)
  return "Task successfully deleted. " + str(size) + " tasks remaining."

def move_task(move_list, from_index, to_index):
  removed_task = move_list.pop(from_index)
  move_list.insert(to_index, removed_task)
  return "Task '" + removed_task + "' successfully moved to index " + str(to_index)

...
item = ''
def add_task(to_do_list, item):
  add = to_do_list.append(item)
  return ('Task successfully added. ' + str(len(to_do_list)) + ' tasks remaining.')

def delete_task(to_do_list, item):
  delete = to_do_list.remove(item)
  return ('Task successfully deleted. ' + str(len(to_do_list)) + ' tasks remaining.')

def move_task(to_do_list, from_index, to_index):
  remove = to_do_list.poplen((from_index))
  reappear = to_do_list.insert(to_index, from_index)
  return ("Task " + str(from_index) + " successfully moved to index " + str(to_index))
  return (to_do_list)
#def move_task(to_do_list, from_index, to_index):
  #pop_value = to_do_list.pop(to_index)
  #from_index = to_do_list.insert(to_index, from_index)
  #return ("Task" + str(from_index) + "successfully moved to" + str(to_index))
  #return (to_do_list)
# ===== YOUR CODE ENDS HERE =====

...
def add_task(lst, task):
    lst.append(task)
    print("Task successfully added. " + str(len(lst)) + " tasks remaining.")
    return lst

def delete_task(lst, task):
    lst.remove(task)
    print("Task successfully deleted. " + str(len(lst)) + " tasks remaining.")
    return lst

def move_task(lst, old_index, new_index):
    task = lst.pop(old_index)
    lst.insert(new_index, task)

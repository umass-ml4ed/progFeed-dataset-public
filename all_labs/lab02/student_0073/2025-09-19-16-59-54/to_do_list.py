...
def add_task(lst, task):
    lst.append(task)
    msg = ("Task successfully added. " + str(len(lst)) + " tasks remaining.")
    return msg

def delete_task(lst, task):
    lst.remove(task)
    msg = ("Task successfully deleted. " + str(len(lst)) + " tasks remaining.")
    return msg

def move_task(lst, old_index, new_index):
    task = lst.pop(old_index)
    lst.insert(new_index, task)
    msg = ("Task " + str(task) + " successfully moved to index " + str(new_index))
    return msg

...
def add_task(x: list, y: str):
    x.append(y)
    return "Task successfully added." + " " + str(len(x)) + " " + "tasks remaining."

def delete_task(x: list, y: str):
   x.remove(y)
   return "Task successfully deleted." + " " + str(len(x)) + " " + "tasks remaining."

def move_task(x: list, from_index, to_index):
   y = x.pop(from_index)
   x.insert(to_index, y)
   return "Task" + " " + y + " " + "successfully moved to index" + " " + str(to_index)

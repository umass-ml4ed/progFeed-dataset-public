...
def add_task(to_do, description):
    to_do.append(description)
    return f"Task successfully added. {len(to_do)} tasks remaining."

def delete_task(to_do, description):
    to_do.remove(description)
    return f"Task successfully deleted. {len(to_do)} tasks remaining."
 
def move_task(to_do, to_move, move_to):
    to_do.insert(move_to, to_do[to_move])
    to_do_list.pop(to_move+1)
    return f"Task {to_do[move_to]} successfully moved to index {move_to}"

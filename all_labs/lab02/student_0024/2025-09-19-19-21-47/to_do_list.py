...
def add_task(to_do_list, taskadd):
  to_do_list.append(taskadd)
  total_tasks = len(to_do_list)
  return "Task successfully added." ,str(total_tasks), "tasks remaining."

def delete_task(to_do_list, taskdel):
  to_do_list.remove(taskdel)
  total_tasks = len(to_do_list)
  return "Task successfully deleted." ,str(total_tasks), "tasks remaining."

def move_task(to_do_list, from_index, to_index):
    task = to_do_list.pop(from_index)      # Remove the item from original position
    to_do_list.insert(to_index, task)      # Insert it at the target position
    return "Task", task, "successfully moved to", str(to_index)

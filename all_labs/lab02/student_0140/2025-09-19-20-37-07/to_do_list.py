...
def add_task(list, a): 
    list.append(a)
    total_a = len(list)
    return f"Task successfully added. {total_a} tasks remaining."

def delete_task(list, a):
        list.remove(a)
        total_a = len(list)
        return f"Task successfully deleted. {total_a} tasks remaining."

def move_task(list, a, b):
        c = list.pop(a)
        list.insert(b, c)
        return f"Task '{c}', successfully moved to index {b}."

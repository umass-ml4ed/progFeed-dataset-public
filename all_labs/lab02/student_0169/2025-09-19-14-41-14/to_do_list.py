...
def add_task(to_do_list,b):
  ...
  to_do_list.append(b)
  count += 1
  return f"Task succesfully added. {count} tasks remaining."
count_del = len(to_do_list)
def delete_task(to_do_list, c):
  if c in to_do_list:
    to_do_list.remove(c)
    count_del -= 1
    return f"Task succesfully deleted. {count_del} tasks remaining."

def move_task(to_do_list, a, d):
  to_do_list[a], to_do_list[d] = to_do_list[d], to_do_list[a]
  return to_do_list

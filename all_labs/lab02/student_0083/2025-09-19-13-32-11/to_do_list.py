...
def add_task(list_to_do, task):
  ...
  N = len(to_do_list)
  return(f"Task succesfully added. {N} tasks remaining.")
...
def delete_task(list_to_do, task):
  ...
  N = len(to_do_list)
  return(f"Task succesfully deleted. {N} tasks remaining.")

def move_task(list_object, from_index, to_index):
  list_object.index(from_index, to_index)
  T = list_object
  J = to_index
  return(f"Task '{T}' succesfully moved to index {J}")

...
def add_task(to_dolist, task):
  to_dolist.append(task)
  return ('Task successfully added.' + ' ' + str(len(to_dolist)) + ' tasks remaining.')
def delete_task(to_dolist, task):
  to_dolist.remove(task)
  return ('Task successfully deleted.' + ' ' + str(len(to_dolist)) + ' tasks remaining.')
def move_task(to_dolist, old_index, new_index):
  task = to_dolist.pop(old_index)
  to_dolist.insert(new_index, task)
  return ('Task ' + "'" + task + "'" + ' successfully moved to index ' + str(new_index) + '.')

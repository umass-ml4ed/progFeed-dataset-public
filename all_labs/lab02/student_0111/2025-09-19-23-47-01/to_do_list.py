...
def delete_task(to_do_list, task):
  ...
  r=len(to_do_list)
  return f"Task successfully deleted. {r} tasks remaining."

def move_task(to_do_list, from_index, to_index):
  n=to_do_list[from_index]
  to_do_list.pop(from_index)
  to_do_list.insert(to_index, n)
  return f"Task {n} successfully moved to index {to_index}"

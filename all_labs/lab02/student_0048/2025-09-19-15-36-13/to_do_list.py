...
def add_task(list, tas):
  list = to_do_list.append(tas)
  num_tasks = len(to_do_list)
  message = "Task successfully added. "+str(num_tasks)+" tasks remaining."
  return message

def delete_task(list, tas):
    list = to_do_list.remove(tas)
    num_tasks = len(to_do_list)
    message = "Task successfully deleted. "+str(num_tasks)+" tasks remaining."
    return message

def move_task(list, from_index, to_index):
   list = to_do_list.insert(to_index, to_do_list[from_index])
   list = to_do_list.pop(from_index+1)
   message = "Task " + str(to_do_list[from_index])+" successfully moved to index " + str(to_index)
   return message

...
to_do_list = []
#"Task successfully added. " + str(len(s)) + " tasks remaining."
# Below please implement the add_task, delete_task, and move_task functions
# ----- YOUR CODE STARTS HERE -----
def add_task(s,x):
  s.append(x)
  print ("Task successfully added. 1 tasks remaining.")
def delete_task(list,x):
  list.remove(x) 
  print ("Task successfully deleted. " + str(len(list)) + " tasks remaining.")
def move_task(list,x,y):
  z = list[x]
  list.pop(x)
  list.insert(y,z)
  print ("Task " + list[y] + " successfully moved to index " + str(y))
# ===== YOUR CODE ENDS HERE =====

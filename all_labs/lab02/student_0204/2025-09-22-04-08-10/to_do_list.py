...
def add_task(list: list, task: str) -> str:
    list.append(task)
    n = len(list)
    return "Task successfully added. " + str(n) + " tasks remaining."

def delete_task(list: list, task: str) -> str:
    list.remove(task)
    n = len(list)
    return "Task successfully deleted. " + str(n) + " tasks remaining."

def move_task(list: list, index: int, to_index: int) -> str:
    list.insert(to_index, list.pop(index))
    n = len(list)
    return "Task '" + str(list[to_index]) + "' successfully moved to index " + str(to_index) + "."
# ===== YOUR CODE ENDS HERE =====

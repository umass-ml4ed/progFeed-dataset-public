...
def delete_task(l, s):
  output = l.remove(s)
  N = len(l)
  return f"Task successfuly deleted. {N} tasks remaining."
def move_task(l, from_index, to_index):
  task = l.pop(from_index)
  move = l.insert(to_index, task)
  return f"Task '{l[from_index]}' successfully moved to index {to_index}"

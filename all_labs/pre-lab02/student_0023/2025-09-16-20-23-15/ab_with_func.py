# Author   : REDACTED
# Email    : REDACTED
# Spire ID : REDACTED


# Below, please implement the ab_func() function as specified in the pre-lab instructions.
# ----- YOUR CODE STARTS HERE -----
def ab_func(a, b):
    """
    Returns a string with the number a repeated b times, 
    followed by the number b repeated a times.
    """
    # Use string multiplication and concatenation to create the desired pattern
    return str(a) * b + str(b) * a
# ===== YOUR CODE ENDS HERE =====

# You can use the code below to help test your functions

# Uncomment the following lines (i.e. remove the # characters on each line)
# to test ab_func()
print(ab_func(5, 3))
print(ab_func(4, 7))

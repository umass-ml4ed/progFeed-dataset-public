...
class Thermostat:
    ...
    def __str__(self,temp=68):
        ordered_times = sorted(self.schedule)
        output = f'Default temperature: {temp} degrees\n'
        for time in ordered_times:
            if time != ordered_times[-1]:
                output += f'{time} {self.schedule[time]}\n'
            else:
                output += f'{time} {self.schedule[time]}\n'
        return output
    def get_target_temperature(self,target,temp=68):
        ordered_times = sorted(self.schedule)
        smaller_times = []

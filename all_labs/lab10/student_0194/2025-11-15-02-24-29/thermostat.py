...
class Thermostat:
    ...
    def get_target_temperature(self, qTime):
        keys = list(sorted(self.schedule.keys()))
        ...
        for t in range(len(keys)):
            ...
            # Find which times qTime is between
                ...
                if (int(qTime[0:2] + qTime[3:5]) >= int(keys[t][0:2] + keys[t][3:5]) and int(qTime[0:2] + qTime[3:5]) < int(keys[t + 1][0:2] + keys[t + 1][3:5])):
                    tTemp = self.schedule[keys[t]]
                    return tTemp
        return tTemp

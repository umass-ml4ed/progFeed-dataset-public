...
class Thermostat:
    ...
    def get_target_temperature(self, qTime):
        ...
        if qTime in timeList:
            toReturn = self.schedule[qTime]
            return int(toReturn) if float(toReturn).is_integer() else float(toReturn)
        else:
        ...
        # elif timeList[0] >= qTime:
        #     toReturn = self.default
        #     return int(toReturn) if float(toReturn).is_integer() else float(toReturn)
        # elif timeList[-1] <= qTime:
        #     toReturn = self.schedule[timeList[-1]]
        #     return int(toReturn) if float(toReturn).is_integer() else float(toReturn)
        # else:
        #     timeList.append(qTime)
        #     timeList.sort()
        #     for time in range(len(timeList[1:-1])):
        #         if timeList[time] == qTime:
        #             toReturn = self.schedule[timeList[time -1]]
        #             return int(toReturn) if float(toReturn).is_integer() else float(toReturn)
        #
        # if qTime < timeList[0]:
        #     return self.default

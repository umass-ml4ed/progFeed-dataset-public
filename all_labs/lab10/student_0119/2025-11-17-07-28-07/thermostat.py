...
class Thermostat:
    ...
    def __str__(self) -> str:
        def fmt(temp: float) -> str:
            return str(int(temp)) if temp.is_integer() else str(temp)

        lines = [f"Default temperature: {fmt(self.default_temperature)} degrees"]
        for t in sorted(self.schedules):
            lines.append(f"{t} {fmt(self.schedules[t])} degrees")
        return "\n".join(lines)
